...
import com.curiez.admin.service.BasicCRUDService;
...
public  class ProductHandler {
   ...
   public Mono<ServerResponse> save(ServerRequest request) {
      final  Mono<ProductDTO> productDTOMono = request.bodyToMono(ProductDTO.class);
        return productDTOMono.flatMap(p -> service.save(p).flatMap( pr ->
                ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).body(fromValue(pr))));
    }

    public Mono<ServerResponse> update(ServerRequest request){
       final Mono<ProductDTO> productDTOMono = request.bodyToMono(ProductDTO.class);
        return productDTOMono.flatMap(p -> service.update(p,p.getId()).flatMap(pr -> ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).body(fromValue(pr)))
    ...
    public Mono<ServerResponse> create(ServerRequest request) {
        final  Mono<ProductDTO> productDTOMono = request.bodyToMono(ProductDTO.class);
        return productDTOMono.flatMap(p -> service.create(p,p.getId()).flatMap(pr -> ServerResponse.created(request.uri()).contentType(MediaType.APPLICATION_JSON).body(fromValue(pr)))
